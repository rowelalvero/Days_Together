...
**The `love_notes` "fix" prescribed above — distinct subscription keys with the `type` filter moved server-side — is not implementable with the installed `supabase` package (2.13.0), and was never actually a bug.** Verified directly against the package source (`supabase_stream_builder.dart`): `SupabaseStreamBuilder` (what `.from(table).stream(...)` returns) stores its `.eq()` filter in a single nullable field, `_StreamPostgrestFilter? _streamFilter`. Calling `.eq()` a second time does not combine two filters — it **overwrites** the first. `.eq('couple_id', coupleId).eq('type', 'chat')`, as this ADR prescribed, would silently drop the `couple_id` scoping entirely and stream every couple's chat rows to every client — a far worse bug than the one it was meant to fix. The same single-filter limit applies one layer down, at `channel().onPostgresChanges(filter: PostgresChangeFilter)` (also a single, non-composable filter object), so bypassing `.stream()` doesn't route around it either. A true compound server-side filter would require a schema change (e.g. two Postgres views, one per `type`) — explicitly out of scope for this migration (see ADR-013's "Should the schema change" — decided no).

Re-examining the original claim with this constraint in hand: **the shared subscription key was never a bug.** `RealtimeSubscriptionManager` keying both `NoteitProvider`'s and `LoveChatProvider`'s successors to the same `'love_notes_$coupleId'` slot is its deduplication feature working exactly as designed — one physical Postgres realtime subscription, two logical consumers, each filtering client-side for the rows it owns. That is the intended behavior of "a logical realtime stream... has exactly one authoritative subscription owner" (Rule B, above) applied to two features that happen to read the same table; it does not violate Rule B, it fulfills it. Rule B's text ("distinct, server-side-filtered subscription keys") is retracted as unachievable; the standing rule going forward is: **two features reading the same table share one subscription key and one manager-level stream, each filtering client-side for its own rows** — exactly what `NoteitController`/`LoveChatController` (Phase 6a) do, unchanged from `NoteitProvider`/`LoveChatProvider`'s original behavior.

The one real, if minor, risk this ADR correctly identified — "a silent third feature could join the shared stream unnoticed" — still stands and is still worth guarding against, just not via a fix that turns out to be infeasible. The mitigation is social/documentary, not mechanical: any future `love_notes`-backed feature's PR should cross-reference this ADR and `NoteitController`/`LoveChatController`'s shared doc comment before assuming a fresh `tableName => 'love_notes'` is uncomplicated.

## Consequences

**Positive:** no regression risk to the one piece of realtime infrastructure that already works correctly. The `love_notes` "collision" turned out not to be a bug at all (see correction above) — no fix was needed, and none was applied.
